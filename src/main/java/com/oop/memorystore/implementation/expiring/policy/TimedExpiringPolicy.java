...
public class TimedExpiringPolicy<V> implements ExpiringPolicy<V, TimedExpiringPolicy.TimedExpirationData> {
    ...
    public static <V> ExpiringPolicy<V, TimedExpirationData> create(final long time, final TimeUnit unit, final boolean shouldResetAfterAccess) {
        return new TimedExpiringPolicy<>($ -> new TimedExpirationData(unit, time, shouldResetAfterAccess));
    }

    public static <V> ExpiringPolicy<V, TimedExpirationData> create(final Function<V, TimedExpirationData> expirationDataFunction) {
