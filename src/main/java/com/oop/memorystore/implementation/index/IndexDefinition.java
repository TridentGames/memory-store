...
public final class IndexDefinition<K, V> {
    ...
    /**
     * Specify a function that maps a given value to a single indexed key. For example, we can index a Person object by its firstName field. Example: IndexDefinition.withKeyMapping(Person::getFirstName) <br>
     * <br>
    ...
    /**
     * Specify a function that maps a given value to a one or more indexed keys. For example, we can index a Person object by both its firstName and lastName fields. Example: IndexDefinition.withKeyMapping(person -&gt; Arrays.asList(person.getFirstName(), person.getLastName()))
     *
    ...
    /**
     * Define the policy for mapping and comparing indexed keys. For example, we can specify a {@link com.oop.memorystore.implementation.index.comparison.string.CaseInsensitiveComparisonPolicy} when we want store and compare indexes insensitive of case.
     *
